...
def exclude(name):
    ...
    if name[-4:] != ".lua":
        return True
    if name[:3] != "cor" and name[:3] != "arm" not in name:
        return True
    return False
...
def _get_folder(data):
    def execute():
        ...
        return result
    contents = json.loads(
        cache.get("f_" + data["name"] + "_" + data["sha"], execute))
...
def get_all_unit_files():
    folder_contents = _get_unit_folder_contents(
        bar_user, bar_repo, bar_units_folder)
